...
export async function pinfo(interaction: ChatInputCommandInteraction<CacheType>, priv: boolean): Promise<void> {
    ...
    if (user) {
        ...
        user.MMRRT = ratingsSuccess && ratings && ratings.mmr_rt > 0 ? ratings.mmr_rt : null;
        user.MMRCT = ratingsSuccess && ratings && ratings.mmr_ct > 0 ? ratings.mmr_ct : null;
        user.MMRVanilla = ratingsSuccess && ratings && ratings.mmr_vanilla > 0 ? ratings.mmr_vanilla : null;
    }
